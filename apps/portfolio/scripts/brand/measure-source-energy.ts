...
const auditRoot = resolve(import.meta.dir, "../../../../.codex/audits/logo-balance");
const outputRoot = resolve(auditRoot, variant);
...
const baselineSource = baselineVariant
  ? await Bun.file(resolve(auditRoot, baselineVariant, "source-energy.json")).json()
  : null;
const baselineRaster = baselineVariant
  ? await Bun.file(resolve(auditRoot, baselineVariant, "metrics.json")).json()
  : rasterMetrics;
