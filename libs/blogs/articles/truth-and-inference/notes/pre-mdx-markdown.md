...
Building on [Vision and Values](/writing/vision-and-values), this
essay leads into [Understanding and Bottlenecks](/writing/understanding-and-bottlenecks) and [The Knowledge
...
is the territory of [Vision and Values](/writing/vision-and-values): what matters, what ought to be
...
can also suggest directions such as `man + royal ≈ king`. The explorer
...
>
> Use this card as a small *engram about engrams*: a compact cue for recalling
> three mechanisms that are often gathered under the metaphor of memory.
>
> - **Biological engram — trace.** A physical change in living neural tissue
>   associated with storing and later reactivating an experience.
>   [Engram research](https://www.nature.com/articles/nrn4000).
...
At the final prompt position, an output projection produces one **logit** per
vocabulary token. **Softmax** converts those scores into
...
weights, see the [training walkthrough in *Vision and Values*](/writing/vision-and-values).
...
The figure above is the article’s core move in miniature. An ambiguous request
leaves a broad distribution — high entropy, many plausible continuations. A
fitting term of art selects a response family, and valid assumptions narrow it
into a handful of testable continuations. The scenario, selected phrase, and
ambiguity meter have separate roles; the meter is an illustrative proxy, not a
measured model probability. The model did not become smarter between the two
prompts; the second prompt simply selected more of the structure the model had
learned.
...
network of algorithmic and language conventions. Expanding that address can
produce a much denser response while preserving the prompt’s direction and
making its inferred choices visible.
...
The figure below states a working hypothesis, not a settled law of language:
domains subjected to repeated correspondence and consequence checks tend to
preserve the distinctions that make those checks efficient. Their terms of art
become compact addresses into a much larger body of inputs, operations,
boundaries, and known failures.
...
At the third truth-linked stage, mathematics, logic, type systems, and
programming languages make relationships explicit enough to compose and
calculate. This makes a fourth step possible: systems can generate synthetic
cases, derive consequences, check proofs, type-check programs, and execute
tests. These operations can supply strong evidence of coherence — and, when
execution is part of the check, operational success — but they do not establish
correspondence by themselves. The formal result must still be regrounded in
observation, measurement, and consequence.
...
generalizations, and proof assistant such as [Lean](https://lean-lang.org/) can
...
the result matters. That consequence becomes a case study in Understanding and Bottlenecks.

The deeper contrast is what the notes for this essay call **evaluative
closure**. A coding task often gives the system enough evidence, constraints,
feedback, and authority to determine whether its change is better — the tests
and benchmarks value the result on the system’s behalf. A strategy task often
asks the system to define “better” while simultaneously guessing the world, the
