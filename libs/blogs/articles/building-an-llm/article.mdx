...
import { Card, CardContent, CardHeader, CardTitle, DecodingExplorer, EmbeddingCompositionExplorer, formatDate, GenerationPlayback, Link, LinkPreview, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger, TrainingWalkthrough } from "./article-components"
...
<Section index="02" title="1. Input: Text Becomes Tokens">
        <P>
          ...
          <Term definition="A reversible mapping between text fragments and integer IDs in a fixed vocabulary.">tokenizer</Term>{" "}
        ...
        <P>
          ...
          <Term definition="A vocabulary-building procedure that repeatedly merges frequent adjacent symbol pairs.">byte-pair encoding</Term>.
        ...
        <P className="article-outline__flow">text → tokenizer → token IDs → tokenizer decoder → text</P>
        <P>
        ...
        <P className="article-outline__flow">P(xₜ | x₁, x₂, …, xₜ₋₁)</P>
...
<Section index="03" title="2. Training: Prediction Error Changes Weights">
        ...
        <table tabIndex={0}>
          <thead><tr><th>Possible next token</th><th>Probability</th></tr></thead>
          <tbody>
        ...
        <P>
          ...
          <Term definition="A score that penalizes probability assigned away from the observed next token.">cross-entropy loss</Term>{" "}
        ...
        <Asset id="training-walkthrough" initialMode="model" autoplay={false} />
        <ol>
        ...
        <P className="article-outline__flow">context → token probabilities → observed token → loss → backpropagation → updated weights</P>
        <P>
...
<Section index="04" title="3. The Model: Learned Weights and Embeddings">
        ...
        <P>
          ...
          production model. That geometry is useful, but it is not a complete theory of meaning and does not
          establish that a relationship is true in the world.
        </P>
        <P>
        ...
        <P className="article-outline__flow">token ID → input embedding → contextual hidden state → output logits → next-token probabilities</P>
        <P>
...
<Section index="05" title="4. Inference: One Token at a Time">
        ...
        <Asset id="generation-playback" autoplay={false} />
        <P className="article-outline__flow">prompt → tokens → representations → logits → probabilities → selected token → append → repeat</P>
        <P>
        ...
        <P>
          ...
          continuations without changing a learned weight. Runtime context and decoding determine which learned
          patterns are activated and how the resulting probabilities become text.
        </P>
      </Section>
<Section index="06" title="5. What Pattern Prediction Means">
        ...
        <P>
          ...
          <ArticleLink slug="understanding-and-bottlenecks">Understanding and Bottlenecks</ArticleLink> argues,
        ...
        <P className="article-outline__flow">
          human language → tokens → training examples → prediction error → learned weights and embeddings →
          prompt-conditioned activations → next-token probabilities → decoded tokens → generated text
        </P>
        <blockquote>
